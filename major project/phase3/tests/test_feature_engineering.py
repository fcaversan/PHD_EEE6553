...
def test_extract_lexical_features():
    ...
    # Should return dict with 27 features
...
def test_extract_features_batch():
    ...
    # Check shape
    assert features_df.shape == (3, 27)
    
    # Check dtype
    assert features_df.dtypes.iloc[0] == np.float32
...
def test_scaler_save_and_load(tmp_path):
    ...
    # Create sample data
    train_data = pd.DataFrame(np.random.randn(100, 27), dtype=np.float32)
    test_data = pd.DataFrame(np.random.randn(10, 27), dtype=np.float32)
