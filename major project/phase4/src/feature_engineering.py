...
Extracts 27 deterministic lexical features from raw URLs:
23 baseline lexical heuristics + 4 brand-aware impersonation features.
"""
...
try:
    from brand_features import extract_brand_features
except ModuleNotFoundError:
    from src.brand_features import extract_brand_features
...
def extract_lexical_features(url: str) -> dict:
    """
    Extract all 27 lexical features from a single URL.
    
    Args:
        url: Raw URL string
    
    Returns:
        dict: Dictionary with 27 feature names and their float32 values
    """
    ...
    # Reserved feature slot (23) - placeholder retained for compatibility
    features['reserved_feature'] = float(0)

    # Brand-aware impersonation features (24-27)
    features.update(extract_brand_features(url))
...
def extract_features_batch(urls: np.ndarray) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: Feature matrix (N, 27) with float32 dtype
    ...
    # Validate output
    if df_features.shape[1] != 27:
        raise ValueError(
            f"Expected 27 features, got {df_features.shape[1]}. "
...
def fit_and_save_scaler(train_features: pd.DataFrame, scaler_path: str) -> StandardScaler:
    ...
    Args:
        train_features: Training feature DataFrame (N, 27)
...
def load_and_apply_scaler(features: pd.DataFrame, scaler_path: str) -> np.ndarray:
    ...
    Args:
        features: Feature DataFrame (N, 27)
        scaler_path: Path to saved scaler (.pkl)
    
    Returns:
        np.ndarray: Scaled features (N, 27) as float32
