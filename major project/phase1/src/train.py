"""
Training pipeline for Malicious URL Detection Model
End-to-end training with callbacks, artifact saving, and visualization
"""
...
def train_model():
    ...
    3. Extract and scale lexical features
    4. Tokenize and pad URL sequences
    5. Build model
    ...
    print("\n" + "="*60)
    print("MALICIOUS URL DETECTION MODEL - TRAINING PIPELINE")
    print("="*60)
    ...
    print(f"\n{'='*60}")
    print("Phase 2: Lexical Feature Extraction")
    print(f"{'='*60}")
    ...
    # Fit scaler on training features only and save
    ...
    print(f"\n{'='*60}")
    print("Phase 3: URL Sequence Processing")
    print(f"{'='*60}")
    
    tokenizer_path = config['data']['tokenizer_path']
    
    # Process training URLs (fit tokenizer, compute max_length, save)
    X_train_sequences, tokenizer, max_sequence_length = \
        process_training_urls(X_train, tokenizer_path, percentile=95)
    
    # Process validation and test URLs (use fitted tokenizer)
    ...
    # Save max_sequence_length for inference
    import json
    metadata = {
    ...
    # ========== Build model ==========
    print(f"\n{'='*60}")
    print("Phase 4: Model Construction")
    print(f"{'='*60}")
    ...
    print(f"\nNext step: Run evaluate.py to generate classification report and confusion matrix")
